...
function signOut(el, periodNum) {
  var criticalTime = new Date(
    el.parentElement.attributes["critical-time"].value
  );
  ...
  var msg =
    "Вы уверены?" +
    (now > criticalTime
      ? "\nCледующая возможность записаться на отработку данной ЛР будет предоставлена Вам " +
        blockUntilTime +
        "!"
      : "");
  if (confirm(msg)) {
